...
//                       exchangesDataContainer.classList.add("exchanges-data-container");
                        ...
                        exchangeDataContent += `<h3>${row.FOR_OR_ON}</h3><h3 class="date">${row.DEBT_DATE}</h3><h3>${numberFormat(row.sum_ammount_new)}</h3><h3>${numberFormat(row.sum_ammount_old)}</h3><h3>${numberFormat(row.sum_ammount_sa)}</h3><textarea name="tDNote" class="note" rows="2" cols="20">${row.NOTE}</textarea></div></div>`;
