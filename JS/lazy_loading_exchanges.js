        ...
        .then(data => {
            ...
            if (data.length === 0) {
                ...
                data.forEach(row => {
                    ...
                    exchangeDataContent += `<h3>${row.TYPE}</h3><h3>${row.SENDER_NAME}</h3><h3>${row.SENDER_PHONE}</h3><h3>${row.RECEIVER_NAME}</h3><h3>${row.RECEIVER_PHONE}</h3><h3>${row.TRANSFER_NO}</h3>`;
                    if (row.TYPE != 'تحويل') {
