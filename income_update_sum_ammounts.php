    ...
    if ($currency == "new") {
        if ($for_or_on == "له") {
            ...
            foreach ($exchangesListData as $traData) {
                if ($traData['INCM_ID'] >= $id) {
                    fwrite($error_file, "exchanges number " . $traData['INCM_ID']."\r\n");
                    ...
                    $stmt = $conn->prepare("UPDATE income SET sum_ammount_new =? WHERE INCM_ID = ?");
                    $stmt->bind_param("di", $sum_ammount_new, $traData['INCM_ID']);
                    if ($stmt->execute()) {
...
//                    $sum_ammount_old += $ammount_differ;
            foreach ($exchangesListData as $traData) {
                if ($traData['INCM_ID'] >= $id) {
                    ...
                    $stmt = $conn->prepare("UPDATE income SET sum_ammount_old =? WHERE INCM_ID = ?");
                    $stmt->bind_param("di", $sum_ammount_old, $traData['INCM_ID']);
                    if ($stmt->execute()) {
//                        echo json_encode(["success" => "تم التعديل بنجاح"]);
                    } else {
                        error_log('حدث خطا اثناء تعديل الاجمال');
...
//                    $sum_ammount_sa += $ammount_differ;
            foreach ($exchangesListData as $traData) {
                if ($traData['INCM_ID'] >= $id) {
                    ...
                    $stmt = $conn->prepare("UPDATE income SET sum_ammount_sa =? WHERE INCM_ID = ?");
                    $stmt->bind_param("di", $sum_ammount_sa, $traData['INCM_ID']);
                    if ($stmt->execute()) {
//                        echo json_encode(["success" => "تم التعديل بنجاح"]);
                    } else {
                        error_log('حدث خطا اثناء تعديل الاجمال');
